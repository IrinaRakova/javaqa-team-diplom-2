...
public class SavingAccount extends Account {
    ...
    public SavingAccount(int initialBalance, int minBalance, int maxBalance, int rate) {
        ...
        if (minBalance > maxBalance) {
            throw new IllegalArgumentException(
                    "Минимальный баланс " + minBalance + " должен быть меньше максимального " + maxBalance
            );
        }
        if (minBalance < 0) {
            throw new IllegalArgumentException(
                    "Минимальный баланс " + minBalance + " не может быть отрицательным"
            );
        }
        if (initialBalance < minBalance) {
            throw new IllegalArgumentException(
                    "Начальный баланс " + initialBalance + " должен быть больше минимального" + minBalance
            );
        }
        if (initialBalance > maxBalance) {
            throw new IllegalArgumentException(
                    "Начальный баланс " + initialBalance + " должен быть меньше максимального" + maxBalance
            );
        }
    ...
    public boolean pay(int amount) {
        ...
        if ((balance = balance - amount) >= minBalance) {
            return true;
        } else {
            return false;
        }
